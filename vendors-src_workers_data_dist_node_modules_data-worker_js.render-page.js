...
const handlers = {
  ...
  summarize_by_group: props => {
    ...
    groupBy(by, [summarize(summarizer)]));
  },
  count: props => {
    const {
      column,
  ...
  tidy: props => {
    ...
    const labels = Object.keys(renaming); // eslint-disable-next-line @typescript-eslint/no-explicit-any
    // todo handle time

    let index = 0;
    const tidied = data ? tidy(data, props.sliceHead ? sliceHead(props.sliceHead) : undefined, props.sliceTail ? sliceTail(props.sliceTail) : undefined, props.sliceMin ? sliceMin(props.sliceMin, props.sliceColumn) : undefined, props.sliceMax ? sliceMax(props.sliceMax, props.sliceColumn) : undefined, mutate({
      index: () => index++
    }), select(labels), rename(renaming)) : [];
    return tidied;
  }
};
